...
int main(int argc, char *argv[]) {
	...
	exit(-1);
      ...
      int nread = read(f, master_data->payload, (int) master_data->payload_length);
      if (nread != master_data->payload_length)
	printf("only read %d of %d from data file\n", nread, (int) master_data->payload_length);
	
      while (nread != 0) {
	tmp = malloc(master_data->total_length * 2);
	if (tmp == NULL) {
	  printf("cannot allocate buffer of %d bytes\n", request_length);
	  exit(-1);
	}
	bcopy(master_data, tmp, master_data->total_length);
	stream_queue_add(out_queue, tmp);
      }
      close(f);
    }
	...
	// pull an "incoming" buffer off the queue
        ...
        // pull an "incoming" buffer off the queue
        buf = stream_queue_get(out_queue);
        /* buf = stream_queue_get(tmp); */
        buf->record_counter = buf_count;
      }
